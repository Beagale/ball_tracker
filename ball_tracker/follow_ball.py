...
class FollowBall(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("extra_forward_distance", 0.5)  # meters to move after reaching target
        self.declare_parameter("extra_forward_speed", 0.1)     # speed used during extra movement (m/s)
        self.declare_parameter("use_odometry_for_extra", False) # If true, use /odom to measure distance
    ...
    def timer_callback(self):
        ...
        if (time.time() - self.lastrcvtime < self.rcv_timeout_secs):
            ...
            # If we're currently executing the extra forward move, do that instead
            if self.extra_move_active:
                # drive straight forward during extra move
                msg.angular.z = 0.0
    ...
    def odom_callback(self, msg):
        # Update current odometry position (for distance measurement)
